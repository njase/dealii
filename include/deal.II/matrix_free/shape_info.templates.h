                 ...
                 const unsigned int base_element_number)
    {
    	...
    	for (int c=0; c<vector_n_components; c++)
	    {
    		if (FEName::FE_Q_TP == fe_name)
    		{
    	    	for (int d=0; d<dim; d++)
	    	    {
    		    	shape_gradients_vec[c][d] = this->base_shape_gradients[0].begin();
    		    	shape_values_vec[c][d] = this->base_shape_values[0].begin();
    	    		shape_hessians_vec[c][d] = this->base_shape_hessians[0].begin();
	    	    }
    		}
    		//else
    			//TBD
